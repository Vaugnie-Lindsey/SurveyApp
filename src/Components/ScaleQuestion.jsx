...
const ScaleQuestion = (props) => {
  ...
  const questions = options.map((answer, index) => (
    <div key={index} className={`${checkIfSelected(answer)} h-20 w-28 p-1 rounded-md transition-all flex flex-col-reverse items-center justify-center`} onClick={() => SetFocus(answer)}>
      <input
        ...
        required
      ></input>
      <label className="text-center" for={props.question + index}>{answer}</label>
    </div>
  ));
  return (
